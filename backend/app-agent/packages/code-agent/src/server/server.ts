...
async function createStaticResponse(filePath: string) {
    const file = Bun.file(filePath)
    if (await file.exists()) {
        return applyStaticHeaders(new Response(file))
    }
    return undefined
}
...
function resolveStaticPath(requestPath: string) {
    ...
    const absolute = path.resolve(STATIC_ASSET_ROOT, trimmed)
    if (!absolute.startsWith(STATIC_ASSET_ROOT)) {
        return undefined
    }
    return absolute
}
