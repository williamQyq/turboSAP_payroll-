...
# =============================================================================
# Output Export Endpoints (for Export Center)
# =============================================================================
...
async def list_all_outputs(
    ...
    """
    List all completed session outputs across all modules.

    Used by Export Center to show config module outputs.
    """
    try:
        all_outputs = session_output_store.get_all_outputs()

        # Enrich with module names
        result = {}
        for module_slug, sessions in all_outputs.items():
            module = module_service.get_module(module_slug)
            module_name = module.name if module else module_slug
...
async def list_module_outputs(
    ...
    """
    List completed session outputs for a specific module.
    """
    try:
        ...
        logger.error(f"Error listing module outputs: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_output(
    ...
    try:
        ...
        if not output:
            raise HTTPException(
                status_code=404,
                detail=f"Output for session '{session_id}' not found",
            )
...
@router.get("/{slug}/outputs/{session_id}/{filename}")
async def download_persisted_output_file(
    slug: str,
    session_id: str,
    filename: str,
    current_user: dict = Depends(get_current_user),
    ...
    """
    Download a specific output file from persisted storage.
    """
    try:
        output = session_output_store.get_output(slug, session_id)
        ...
        if filename not in output["files"]:
            raise HTTPException(
                status_code=404,
                detail=f"File '{filename}' not found. Available: {list(output['files'].keys())}",
            )

        content = output["files"][filename]
...
async def delete_output(
    ...
    """
    Delete persisted output for a session. Admin only.
    """
    try:
